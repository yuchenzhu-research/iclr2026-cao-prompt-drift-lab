...
- v1_paraphrase_judge and v2_schema_strict_judge are supported and intended to be reproducible.
- v0_baseline_judge is legacy and NOT reproducible (historical bundle + model-name inconsistencies).
  The repository keeps v0 artifacts as provided; do not treat regenerated v0 outputs as authoritative.

This script is offline and deterministic. It does NOT run any model inference.
"""
...
def main():
    ...
    parser.add_argument(
        "--include-v0",
        action="store_true",
        help="Include v0_baseline_judge (legacy / not reproducible). Not recommended for artifact reproduction.",
    )
    parser.add_argument(
        "--runs",
        nargs="*",
        default=["v1_paraphrase_judge", "v2_schema_strict_judge"],
    ...
    for r in args.runs:
        if r == "v0_baseline_judge" and not args.include_v0:
            print("[SKIP] v0_baseline_judge is legacy and not reproducible. Use --include-v0 to run anyway.")
            continue
