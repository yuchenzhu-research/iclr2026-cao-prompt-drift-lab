...
Our analysis is conducted under a fixed evaluation snapshot, with results derived from processed evaluation records and aggregate summaries.
Across six preserved judge bundles under a baseline judge (three judge models scoring outputs from the other two providers), reducing contract salience from explicit to implicit increases schema hard-breaks (\Astruct$=0$) from $8/48$ (16.7\%) to $34/48$ (70.8\%) and drops the mean total score from 7.69 to 2.75 (max total = 10).
We further show judge-version sensitivity on \emph{identical preserved outputs}: for one overlapping slice (ChatGPT-judge scoring Gemini outputs under implicit triggers), the hard-break rate shifts from $8/8$ (v0) to $0/8$ (v1/v2), and the mean total score shifts from 0.0 (v0) to 8.0 (v1) to 10.0 (v2).
...
\caption{\textbf{Evaluation pipeline overview.} A visual summary of the one-way evidence chain from prompt variants to raw outputs, judge bundles, processed records, and summary tables.}
...
\begin{itemize}[leftmargin=1.2em, itemsep=2pt, topsep=2pt]
  ...
  \item \textbf{Artifact-grounded evaluation and auditability.} We treat scoring rules as change-controlled configuration and report results from a fixed evaluation snapshot; all evidence needed for review appears in this PDF, and quantitative claims follow a one-way evidence chain from prompts to outputs to summaries.
  \item \textbf{Risk patterns under minimal perturbations.} Using preserved outputs and versioned judges, we surface schema failure cliffs, broad instruction-following degradation, and judge-version sensitivity as concrete operational risks.
\end{itemize}
...
All quantitative claims are sourced from computed from the fixed snapshot and reported in this paper:
...
\end{itemize}
\noindent\textbf{Note.} These summaries are internal processing artifacts; all evidence needed for review appears in this PDF (Figures~1--4 and Table~1).
stored under the processed evaluation directories for each judge version.
...
\[
\text{prompt variants} \;\rightarrow\; \text{raw outputs} \;\rightarrow\; \text{judge bundles} \;\rightarrow\;
\text{processed records} \;\rightarrow\; \text{summary tables}.
\]
Later stages are derived views and must not mutate earlier artifacts.
This design makes quantitative claims auditable: reported aggregates can be expanded to row-level records, which in turn link back to preserved raw outputs and judge bundles.
...
Across six preserved judge bundles under v0 (48 scored files per trigger type; 6 bundles $\times$ 8 preserved outputs), the schema hard-break rate jumps from $8/48$ (16.7\%) under explicit triggers to $34/48$ (70.8\%) under implicit triggers.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.32\linewidth]{figures/fig3_dimension_breakdown_chatgpt.pdf}
  \includegraphics[width=0.32\linewidth]{figures/fig3_dimension_breakdown_claude.pdf}
  \includegraphics[width=0.32\linewidth]{figures/fig3_dimension_breakdown_gemini.pdf}
  ...
  \includegraphics[
    ...
    keepaspectratio,
    trim=0 0 0 6, clip
  ]{figures/fig4_dimension_failure_rate.pdf}
  \vspace{-2pt}
  \caption{\textbf{Dimension‑level degradation under implicit triggers (v0).}
  Top: mean scores by dimension for ChatGPT, Claude, and Gemini (left to right).
  Bottom: failure rates (score $<2$) by dimension aggregated across generator families.
  Numbers sourced from the fixed snapshot under v0.
  Top panels are computed as $\mu_d(\pi)$ (Eq.~\ref{eq:dim-mean}); bottom panels as $\mathrm{DFR}_d(\pi)$ (Eq.~\ref{eq:dim-fail}) over frozen artifacts.
}
  \label{fig:dim_pair}
\end{figure*}
...
We highlight one overlapping slice with complete totals: \texttt{judge\allowbreak\_chatgpt\allowbreak\_bundle\allowbreak\_gemini\allowbreak.json} under implicit triggers.
\end{sloppypar}
...
In the fixed snapshot, v1/v2 are available for two overlapping judge bundles---\texttt{judge\allowbreak\_chatgpt\allowbreak\_bundle\allowbreak\_gemini} and \texttt{judge\allowbreak\_gemini\allowbreak\_bundle\allowbreak\_chatgpt}---while other bundles appear only under v0.
...
Our analysis of the preserved records supports three primary claims for practitioners:
(i) minimal reductions in contract salience can trigger \emph{discontinuous} interface validity failures (\Astruct$=0$);
(ii) monitoring should be slice-aware, as worst-slice validity/quality can diverge from global averages and judge prompt edits can create measurement incidents, requiring versioned instrumentation.
...
Using a fixed-snapshot evidence base, we show that small prompt perturbations can trigger schema failure cliffs, degrade rubric-dimension scores, and shift evaluation conclusions when judge prompts change.
...
\section*{Reproducibility}\vspace{-2pt}
Claims are based on frozen evaluation slices (Q3--Q4) and supported by the audit chain provided in Section~4 and Figure~1. 
While LLMs assisted in generation, evaluation, and text refinement, all findings remain anchored to and verified against the preserved outputs from the fixed snapshot.
